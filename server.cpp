...
static Conn *handle_accept(int fd) {
  ...
  char ip_str[INET_ADDRSTRLEN]; // Buffer to hold the string (usually 16 bytes)
...
static bool try_one_request(Conn *conn) {
  ...
  conn->outgoing.append((const uint8_t*)&len, k_header_size);
...
static void handle_read(Conn *conn) {
  ...
  if (conn->outgoing.size() > 0) {
    ...
    // optimistic write: call handle_write immediately to save one poll()
    // syscall. using 'return' enables tail-call optimization, allowing the CPU
    // to jump directly back to the event loop, skipping the return 'hop'
...
int main() {
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val)) <
      0) { // socket option to allow same ip:port after restart
    die("setsockopt()");
  }
  fd_set_nonblock(fd);

  struct sockaddr_in addr =
      {}; // address to bind to the socket that the server will listen on
  addr.sin_family = AF_INET;
  addr.sin_port = htons(1234);
  addr.sin_addr.s_addr = htonl(0);
  if (bind(fd, (const struct sockaddr *)&addr, sizeof(addr)) <
      0) { // // binds the socket to this address
    die("bind()");
  }

  if (listen(fd, SOMAXCONN) <
      0) { // server turns on and starts listening for incoming connections
    die("listen()");
  }
